...
<title>Reset password · Corez.io</title>
<style nonce="${nonce}">
...
<h1>Reset password</h1><p id="who">Choose a new password for your Corez.io account.</p>
    ...
    fetch('/api/auth/reset', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: token, newPassword: a }) })
      ...
      .then(function(res){
        if (res.ok) { f.hidden = true; say('Password changed! You are logged in' + (res.j.user ? ' as ' + res.j.user.username : '') + '.', 'ok');
          var a2 = document.createElement('a'); a2.href = '/'; a2.textContent = 'Play Corez.io'; msg.appendChild(document.createElement('br')); msg.appendChild(a2); return; }
